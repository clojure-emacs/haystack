...
*Note:* much of the functionality in Haystack is now available as part of
[[https://github.com/clojure-emacs/orchard][Orchard]], in particular
[[https://github.com/clojure-emacs/orchard/blob/master/src/orchard/stacktrace.clj][orchard.stacktrace]].
We recommend trying Orchard first, as it's more actively maintained. If you need
to parse stacktraces from strings, then Haystack is still the way to go.

** Haystack vs =orchard.stacktrace=

The =orchard.stacktrace= namespace is a repatriation of Haystack's
analyzer back into Orchard (see [[https://github.com/clojure-emacs/orchard/pull/322][orchard#322]]). When deciding between the
two, keep these differences in mind:

- *Analyzer:* Both provide an =analyze= function that turns a
  =Throwable= (or a map in the =Throwable->map= format) into an
  enriched cause chain. Orchard's version was modernized and
  optimized; most notably its project-frame detection runs in under a
  millisecond, where Haystack's namespace scan can take ~100ms on
  large projects.

- *Parser:* This is the key distinction. Haystack also ships a parser
  that reconstructs stacktrace data from a *printed* stacktrace (REPL
  output, a log file, an exception buried in another string).
  =orchard.stacktrace= has no equivalent and only works with live
  exceptions or their =Throwable->map= data.

In short: reach for =orchard.stacktrace= when you have the exception
object at hand, and for Haystack when all you have is its printed text.
...
success, the parse result is an enhanced version of the Clojure data
...
- Most of the parsers in Haystack are implemented with [[https://github.com/Engelberg/instaparse][Instaparse]] and
  ...
  come up with an Instaparse grammar for the new stacktrace format as
